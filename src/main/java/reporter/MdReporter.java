...
public class MdReporter implements IReporter{
	...
	public int createReport(String path, ArrayList<Task> taskList) {
		try {
			...
			outputStream.write("# *"+"TaskId	TaskText	MamaId	Start	End	Cost"+"*<br>\n");
			for (Task task : taskList) {
				outputStream.write("**"+task.toString()+"**<br>\n");
				if(task.getChildTaskList()!=null) {
					for(Task child : task.getChildTaskList()) {
						outputStream.write(child.toString()+"<br>\n");
					}
				}
			}
			
		} catch (IOException e) {
			System.err.println("Problem with opening, writing to or closing output file stream while trying to create markdown report");
			e.printStackTrace();
			return 0;
		}
